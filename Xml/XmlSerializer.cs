...
namespace Squid.Xml
{
    ...
    public class XmlSerializer : IDisposable
    {
        ...
        private void Write(Node node, XmlWriter writer)
        {
            ...
            if (node.WriteType)
            {
                writer.WriteAttributeString("xtype", node.Type);
            }

            if (!string.IsNullOrEmpty(node.Key))
            {
                writer.WriteAttributeString("xkey", node.Key);
            }

            if (node.RefID != 0)
            {
                writer.WriteAttributeString("xrefid", node.RefID.ToString());
            }

            if (node.Reference != 0)
            {
                writer.WriteAttributeString("xref", node.Reference.ToString());
            }
        ...
        private object DeserializeTo(object target, Node node, Type propertyType)
        {
            if (node.Reference != 0)
            {
                ...
                {
                    var refnode = FindNode(Root, node.Reference);
                    if (refnode != null)
                    {
